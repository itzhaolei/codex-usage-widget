...
swiftc -parse-as-library -o "$BUILD_DIR/QuotaStoreTests" \
    "$ROOT/sources/QuotaModels.swift" "$ROOT/sources/QuotaStore.swift" "$ROOT/tests/QuotaStoreTests.swift" \
    -framework Cocoa -framework Combine
"$BUILD_DIR/QuotaStoreTests"

swiftc -parse-as-library -o "$BUILD_DIR/QuotaBubble" \
    "$ROOT/sources/QuotaModels.swift" "$ROOT/sources/QuotaStore.swift" "$ROOT/sources/QuotaBubbleApp.swift" \
