...
압축을 푼 뒤 `Install Quota Bubble.app`을 더블 클릭하세요. Quota Bubble은 현재 사용자의 로컬 Codex 할당량 데이터를 읽습니다. 로컬 Codex CLI 데이터가 아직 없으면 설정 오버레이가 Codex CLI 설치와 로그인을 안내합니다.

Windows는 현재 v2.1.3을 유지합니다. [Windows v2.1.3 릴리스](https://github.com/itzhaolei/codex-usage-widget/releases/tag/v2.1.3)에서 `QuotaBubble-*-Windows.zip`을 다운로드하고 압축을 푼 뒤 `windows/install.ps1`을 실행하세요.
