...
macOS アプリは Swift でネイティブに使用量を取得します。Node.js、npm、別途インストールした Codex CLI、Xcode、コマンドラインツールは不要で、macOS 13 以降、ログイン済みで `~/.codex/auth.json` を生成した Codex、Codex へのネットワーク接続だけが必要です。

Windows は macOS と同じ v3.0.4 です。[最新リリース](https://github.com/itzhaolei/codex-usage-widget/releases/latest)から `QuotaBubble-*-Windows-Setup.exe` をダウンロードし、ダブルクリックして GUI インストーラーを実行してください。PowerShell、Node.js、ターミナル、別途の .NET ランタイムは不要です。
