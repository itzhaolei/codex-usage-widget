...
解凍して `Install Quota Bubble.app` をダブルクリックしてください。Quota Bubble は現在のユーザーのローカル Codex quota データを読み取ります。ローカルの Codex CLI データがまだ利用できない場合は、セットアップオーバーレイで Codex CLI のインストールとログインを案内します。

Windows は当面 v2.1.3 のままです。[Windows v2.1.3 リリース](https://github.com/itzhaolei/codex-usage-widget/releases/tag/v2.1.3) から `QuotaBubble-*-Windows.zip` をダウンロードし、解凍して `windows/install.ps1` を実行してください。
