...
- macOS のメニューバーでは 5 時間クォータがある場合はそれを優先し、ない場合は週間クォータの残量をリアルタイム表示。
- 閉じる操作ではウインドウだけを非表示にし、メニューバーまたはシステムトレイのクォータ更新は継続します。終了はアプリまたはトレイメニューから行います。
...
- アプリメニューまたは `Command-N` で同期ウインドウを複数開けます。すべて同じライブクォータを共有し、位置は個別に保存します。
